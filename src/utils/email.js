...
// Security: escape HTML entities to prevent XSS injection via user-supplied data in emails
  ...
  try {
    if (process.env.RESEND_API_KEY) {
      ...
      if (mailOptions.replyTo) {
        resendOptions.reply_to = mailOptions.replyTo;
      }
      if (mailOptions.attachments) {
        resendOptions.attachments = mailOptions.attachments;
      }
      
      const { data, error } = await resend.emails.send(resendOptions);
      if (error) {
        console.warn('Resend failed, falling back to Nodemailer:', error);
        throw new Error('Resend failed'); // trigger fallback
      }
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #E6C587; border-radius: 8px; overflow: hidden;">
          ...
          <div style="padding: 40px 30px; background-color: #fdfaf5; text-align: center;">
            <p style="font-size: 18px; color: #BD561A; font-weight: bold; margin-bottom: 20px;">Dear ${escapeHtml(name) || 'Friend'},</p>
          ...
          <div style="padding: 30px; background-color: #fdfaf5;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
              <tr style="border-bottom: 1px solid #e0e0e0;">
                ...
                <td style="padding: 12px 0; color: #555;">${escapeHtml(leadData.companyName) || 'N/A'}</td>
              </tr>
              <tr style="border-bottom: 1px solid #e0e0e0;">
                <td style="padding: 12px 0; font-weight: bold; color: #BD561A;">Contact Person:</td>
                <td style="padding: 12px 0; color: #555;">${escapeHtml(leadData.contactPerson) || 'N/A'}</td>
              </tr>
              <tr style="border-bottom: 1px solid #e0e0e0;">
                <td style="padding: 12px 0; font-weight: bold; color: #BD561A;">Email Address:</td>
                <td style="padding: 12px 0; color: #555;"><a href="mailto:${escapeHtml(leadData.email)}" style="color: #BD561A; text-decoration: none;">${escapeHtml(leadData.email)}</a></td>
              </tr>
              <tr style="border-bottom: 1px solid #e0e0e0;">
                <td style="padding: 12px 0; font-weight: bold; color: #BD561A;">Phone Number:</td>
                <td style="padding: 12px 0; color: #555;">${escapeHtml(leadData.phone) || 'N/A'}</td>
              </tr>
              <tr style="border-bottom: 1px solid #e0e0e0;">
                <td style="padding: 12px 0; font-weight: bold; color: #BD561A;">Business Type:</td>
                <td style="padding: 12px 0; color: #555;">${escapeHtml(leadData.businessType) || 'N/A'}</td>
              </tr>
              <tr style="border-bottom: 1px solid #e0e0e0;">
                <td style="padding: 12px 0; font-weight: bold; color: #BD561A;">Estimated Volume:</td>
                <td style="padding: 12px 0; color: #555;">${escapeHtml(leadData.estimatedVolume) || 'N/A'}</td>
              </tr>
            </table>
            ...
            <div style="background-color: #fff; border-left: 4px solid #BD561A; padding: 15px; margin-top: 25px;">
              ...
              <p style="margin: 0; color: #555; white-space: pre-wrap; font-style: italic;">${escapeHtml(leadData.notes) || 'No notes provided.'}</p>
            </div>

            <div style="text-align: center; margin-top: 35px;">
              <a href="mailto:${escapeHtml(leadData.email)}" style="background-color: #BD561A; color: #fff; padding: 12px 25px; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 14px; letter-spacing: 1px;">REPLY TO CUSTOMER</a>
            </div>
          </div>
          <div style="background-color: #f4e4cf; padding: 15px; text-align: center; font-size: 12px; color: #888;">
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #E6C587; border-radius: 8px; overflow: hidden;">
          ...
          <div style="padding: 40px 30px; background-color: #fdfaf5; text-align: center;">
            <p style="font-size: 18px; color: #BD561A; font-weight: bold; margin-bottom: 20px;">Dear ${escapeHtml(name) || 'Valued Customer'},</p>
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #E6C587; border-radius: 8px; overflow: hidden;">
          ...
          <div style="padding: 40px 30px; background-color: #fdfaf5; text-align: center;">
            <p style="font-size: 18px; color: #BD561A; font-weight: bold; margin-bottom: 20px;">Dear ${escapeHtml(name) || 'Customer'},</p>
            <p style="font-size: 16px; margin-bottom: 20px;">We have successfully received your payment of <b>Γé¼${totalAmount}</b> for Order #${orderId.substring(0,8).toUpperCase()}.</p>
